...
orgdict["Totals"] = pd.DataFrame(columns=["Year", "Please Indicate The Amount Of Money Given By Clipper Ship", "Please Indicate The Amount Of Money Used By Clipper Ship", "Please indicate the number of Opportunity Youth (ages 16-24) placed into employment in FY' 23:",
                                 "As appropriate, please indicate the number of Opportunity Youth successfully placed into supported or permanent housing in FY' 23:", "Please Indicate Number Of Youth In Program", "Please Indicate Number Of Opportunity Youth achieving different educational milestones in FY' 23, such as HS diploma, GED, college matriculation"])
totaldict = dict()
...
for file in files:
    ...
    overalldf = pd.DataFrame(overalldata, columns=[
    ...
    if specsdf['Value'][0] not in orgdict.keys():
        ...
        orgdict.update(
            {name: orgdict.get(name).rename(columns={'Value': 'Year'})})
        if int(specsdf['Value'][3]) in totaldict.keys():
            print("YO", totaldict)
            for i in range(overalldf.shape[0]):
                orgdict.get(name)[overalldf['Question'][i]] = [
                            int(overalldf["Value"][i])]
                totaldict.get(int(specsdf['Value'][3]))[
                              i] += int(overalldf["Value"][i])
        else:
            totaldict[int(specsdf['Value'][3])] = []
            for i in range(overalldf.shape[0]):
                orgdict.get(name)[overalldf['Question'][i]] = [
                            int(overalldf["Value"][i])]
                totaldict.get(int(specsdf['Value'][3])).append(
                    int(overalldf['Value'][i]))
...
for key in orgdict.keys():
    for j in range(overalldf.shape[0]):
        if len(orgdict.get(key)) > 1:  # can use this line if you want to remove plots for companies with only one year
            plt.plot(orgdict.get(key)['Year'], orgdict.get(
                key)[overalldf['Question'][j]])
